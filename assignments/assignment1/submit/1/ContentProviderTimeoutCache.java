 ...
 */
public
class ContentProviderTimeoutCache
        implements TimeoutCache< String, List< AcronymExpansion > >
{
     ...
     */
    public
    ContentProviderTimeoutCache( Context context )
    {
        // Store the context.
        mContext = context;
     ...
     */
    public
    List< AcronymExpansion > get( final String acronym )
    {
        ...
        try ( Cursor cursor = mContext.getContentResolver().query(  AcronymEntry.CONTENT_URI,
                                                                    null,
                                                                    SELECTION_ACRONYM,
                                                                    selectionArgs,
                                                                    null ))
        {
            ...
            {
                // TODO -- replace "0" with the expiration time of given acronym that's obtained from the cursor.
                //
     ...
     */
    private
    AcronymExpansion getAcronymExpansion( Cursor cursor )
    {
                   ...
                   long                     timeout )
    {
        ...
        for ( int i = 0; i < longForms.size(); i++ )
        {
            ...
            cvs.put( AcronymEntry.COLUMN_EXPIRATION_TIME,   longForms.get(i).getSince() );
        ...
        return mContext.getContentResolver().bulkInsert( AcronymEntry.CONTENT_URI, cvArray );
    }
    ...
    void remove( String acronym )
    {
                                                                    ...
                                                                    null ))
        {
            ...
            {
                // Get the acronym to delete.
                //
                final String str = cursor.getString( cursor.getColumnIndex( AcronymEntry.COLUMN_ACRONYM ) );
                remove( str );
            }
        }
    }
     ...
     */
    public
    void removeExpiredAcronyms()
    {
        // Selection clause to delete acronym expansions that have expired.
        //
        ...
        try ( Cursor expiredData = mContext.getContentResolver().query( AcronymEntry.CONTENT_URI,
                                                                        ...
                                                                        null ))
        {
            // Use the expired data id's to delete the designated entries from both tables.
            //
            if ( expiredData != null && expiredData.moveToFirst() )
            {
                do
                {
                    // Get the acronym to delete.
                    //
                    final String acronym = expiredData.getString( expiredData.getColumnIndex( AcronymEntry.COLUMN_ACRONYM ) );
                    remove( acronym );
     ...
     */
    private
    void scheduleCacheCleanup( Context context )
    {
        // Only schedule the Alarm if it's not already scheduled.
        //
        if ( !isAlarmActive( context ) )
        {
            // Schedule an alarm after a certain timeout to start a service to delete expired data from Database.
            //
            mAlarmManager.setInexactRepeating ( AlarmManager.ELAPSED_REALTIME_WAKEUP,
                                                SystemClock.elapsedRealtime() + CLEANUP_SCHEDULER_TIME_INTERVAL,
     ...
     */
    private
    boolean isAlarmActive( Context context )
    {
