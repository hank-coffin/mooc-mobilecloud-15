                   ...
                   GenericAsyncTaskOps< String, Void, List< AcronymExpansion > >
{
    ...
    void onConfiguration( Activity activity, boolean firstTimeIn )
    {
        ...
        if ( firstTimeIn )
        {
            ...
            mAcronymCache = new ContentProviderTimeoutCache ( activity.getApplicationContext() );

            // Create a proxy to access the Acronym web service.
            // TODO -- you fill in here, replacing "null" with the appropriate initialization of the proxy.
            //
     ...
     */
    public
    void expandAcronym( final String acronym )
    {
        if ( mAsyncTask != null )
        {
            // Cancel an ongoing operation to avoid having two requests run concurrently.
            //
     ...
     */
    public
    List< AcronymExpansion > doInBackground( String acronym )
    {
        try
        {
            ...
            else
            {
                ...
                List< AcronymData > result = mAcronymWebServiceProxy.getAcronymResults( acronym  );

                // Get the "long forms" of the acronym expansion.
                longForms = result.get( 0 ).getLfs();

                // Put the long forms into the cache using the "short
                // form" of the acronym.
                mAcronymCache.put( result.get( 0 ).getSf(), longForms );
     ...
     */
    public
    void onPostExecute( List< AcronymExpansion > acronymExpansionsList, String acronym )
    {
        ...
        mActivity.get().displayResults( acronymExpansionsList, "no expansions for " + acronym + " found" );
